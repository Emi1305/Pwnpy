...
def toUpper_decoder(l):
    ''' Generates the payload decoder for the encoder
    'encode_toUpper_resistant' based on the length of the shellcode '''
...
def encode_toUpper_resistant(sc, add_decoder=True):
    ''' Encodes the shellcode so it's resistant to string.upper functions.
    [add_decoder] prepends the shellcode decoder (default)
    '''
    shellcode = b''
    if add_decoder:
...
def decode_toUpper_resistant(sc):
    ''' Decodes shellcode encoded with the 'encode_toUpper_resistant' encoder
